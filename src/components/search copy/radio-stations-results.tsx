...
function RadioStationsResults(params: { searchResults: RadioStationsSearchResponse}) {
...
function getPagination(searchTitle: string, number: number, totalPages: number) {
    ...
    if (number <= 0) {
        ...
        buttons.push((
            <a key="prev" href={baseLink + '?page=' + (number - 1)} className="mx-1 px-3 py-2 bg-white text-gray-700 hover:bg-blue-500 hover:text-white font-medium rounded-md">
                Previous
            </a>
        ));
    }
    ...
    if (number >= totalPages) {
        ...
        buttons.push((
            <a key="next" href={baseLink + '?page=' + (number + 1)} className="mx-1 px-3 py-2 bg-white text-gray-700 hover:bg-blue-500 hover:text-white font-medium rounded-md">
                Next
            </a>
        ));
    }
