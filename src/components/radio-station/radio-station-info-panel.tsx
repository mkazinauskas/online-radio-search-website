...
function RadioStationInfoPanel(params: {
    ...
    const { radioStationResponseHolder, radioStationStreamsResponseHolder } = params;
    ...
    if (radioStationResponseHolder.error || radioStationStreamsResponseHolder.error) {
        return (
...
function _streams(streams: RadioStationStream[]) {
    const streamsTable = streams.map(stream => (
        <li className="flex items-center space-x-3 hover:bg-gray-100" key={stream.id}>
            ...
            <div className="hidden lg:block lg:flex-1">{stream.url}</div>
            <div className="flex-1">{stream.bitrate}</div>
