...
const facebookIconDescriptor: BottomLink = {
    text: 'Facebook',
...
const instagramIconDescriptor: BottomLink = {
    text: 'Instagram',
...
const twitterIconDescriptor: BottomLink = {
    text: 'Twitter',
...
const githubIconDescriptor: BottomLink = {
    text: 'GitHub',
...
const dribbleIconDescriptor: BottomLink = {
    text: 'Dribble',
...
function Footer(params: { websiteConfig: WebsiteConfig, lastSearhesResponseHolder: ApiResponseHolder<LastSearchesResponse> }) {
    const { websiteConfig, lastSearhesResponseHolder } = params;

    if (!websiteConfig) {
        throw Error('Website config does not exist');
    }

    if (!lastSearhesResponseHolder) {
        throw Error('Last searches do not exist!')
    }

    const { contactUsLink, facebookLink, instagramLink, twitterLink, githubLink, dribbleLink, websiteName } = websiteConfig;
        ...
        <footer className="bg-white">
            ...
            <div className="max-w-screen-xl px-4 py-12 mx-auto space-y-8 overflow-hidden sm:px-6 lg:px-8">
                <nav className="flex flex-wrap justify-center -mx-5 -my-2">
                    ...
                    <div className="px-5 py-2">
                        <Link href={URL_PATH} >
                            <a className="text-base leading-6 text-gray-500 hover:text-gray-900">Privacy Policy</a>
                        </Link>
                    </div>
                </nav>
                <div className="flex justify-center mt-8 space-x-6">
                    {createLink(facebookLink ? facebookLink : contactUsLink, facebookIconDescriptor)}

                    {createLink(instagramLink, instagramIconDescriptor)}

                    {createLink(twitterLink, twitterIconDescriptor)}

                    {createLink(githubLink, githubIconDescriptor)}

                    {createLink(dribbleLink, dribbleIconDescriptor)}
                </div>
                <p className="mt-8 text-base leading-6 text-center text-gray-400">
                    &copy; {new Date().getUTCFullYear()} {websiteName}. All rights reserved.
        </p>
            </div>
        </footer>
...
function createLink(url: string, iconDescriptor: BottomLink) {
    if (!url) {
        return <></>
    }
    return <Link href={url} >
        <a target="_blank" className="text-gray-400 hover:text-gray-500">
            <span className="sr-only">{iconDescriptor.text}</span>
